...
In this extension, we explore what we can expect for confidence interval coverage when we are _not_ operating in a best case scenario. We simulate 1) error in the measurement of the variables on which we rake, and 2) error in the population estimates towards which we rake. We observe how these errors affect confidence interval coverage when the errors occur in isolation, as well as simultaneously. To illustrate when these errors might occur in an applied context, imagine researchers are trying to predict voter turnout in Florida based on a survey of Florida residents. The researchers ask survey respondents about their past voting behavior, their current voter registration status, and whether they anticipate voting in the upcoming election. They also ask several demographic questions for weighting purposes. Respondents may misreport their income due to on-the-spot estimation error or stigma associated with being in a low income bracket (https://www.nap.edu/read/10206/chapter/8#166). If enough respondents misreport their income, and researchers weight on income, they will face error #1 above (or: in the measurement of one of the variables on which they are raking). Error #2 could occur if the researchers weight towards population income estimates based on the American Community Survey (ACS) - a continuous survey by the United States Census Bureau. While the ACS follows rigorous methodology, it is still a survey, and thus has error in some of its estimates. Weighting towards income estimates from the ACS will introduce error #2 above. As seen in this example, the errors can easily occur simulataneously. 
