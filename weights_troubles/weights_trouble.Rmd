...
We passed the `dat_design` object to `as.svrepdesign()` with `type = "bootstrap"` specified; the resulting object was used with `svyglm()` to calculate bootstrapped standard errors. These are known as "replicate" weights in survey literature (CITE, XXXX); the method is more broadly referred to as resampling (CITE, XXXX). Bootstrapping involves drawing *R* resamples from the sample, with replacement, and calculating the estimates for each resample. The standard deviation of the estimates across resamples is then considered to be the standard error of the estimate. We employed the default *R* = 50 here. See Canty and Davison (1999) for the specific bootstrapping implementation used here, and Kline (2004, Ch. 9) for a broader introduction to resampling methods, including boostrapping.  

For each simulation iteration, we calculated the estimate for each model and a logical value for all models (but the population model) indicating whether or not the 95% CI for the estimate contained the population value:  
...
The only model that did not approximate the population estimate in the long-run was the model calculated with the non-response bias and _no_ weighting (see table below), supporting Lumley's (2010) contention that weighted estimates—regardless of standard error calculation—will tend to be unbiased. All weighted models also showed the exact same estimates—these methods only differed in their standard error calculations.  
...
But we do not have the luxury of having the long-run average in applied research—we have just one of these iterations. We would like to use a method that has a narrow spread—where we rarely stray too far away from the population value. Pictured below are density plots of estimates over all 10,000 simulations. The top panel shows how much the estimate differed in the actual population values, while the panel directly below it shows what we see from a sample drawn randomly from it, with no non-response bias. The bottom three distributions are from the weighted models, and again we see that they are all the same—what will differ are their standard errors.  
...
Are the CIs properly characterizing uncertainty? Remember: If we run an experiment a large number of times, 95% of the 95% CIs will contain the true population estimate. We can test if this is the case by testing the coverage of each model—what percentage of the time did the 95% CI cover the population estimate?  
...
Even though the confidence intervals still tended to underestimate uncertainty, these simulations were still somewhat optimistic—they represented a best case scenario for various reasons. First, there was no error in measuring the variables on which we raked on—they were perfect measurements; second, we had the exact population estimates—this is generally the case when we have a pre-defined list of people with which we want to survey, but sometimes only estimated population totals are available; and third, we were weighting on exactly the variables—and only the variables—that relate to both non-response and outcome. In applied situations, these will not hold, and thus we think that we should be even less confident that 95% CIs are valid—we should be more skeptical than usual when thinking about these bounds.  
