...
sand_preds <- data.frame(
  p = seq(0, .15, length.out = 200),
  estimator = "sandwich",
  coverage_error = predict(
    sand, 
    data.frame(
      p_wrong = 0, 
      p_error = seq(0, .15, length.out = 200)
    ),
    type = "response"
  ),
  coverage_wrong = predict(
    sand, 
    data.frame(
      p_wrong = seq(0, .15, length.out = 200), 
      p_error = 0
    ),
    type = "response"
  )
...
boot_preds %>% 
  ...
  gather("key", "coverage", coverage_error:coverage_wrong) %>% 
  ggplot(aes(x = p, y = coverage, color = estimator)) +
  geom_line(size = 1) +
  facet_wrap(~ key, labeller = as_labeller(
    c(coverage_error = "Measurement Error (Perfect Targets)", 
      coverage_wrong = "Target Error (Perfect Measurement)")
  )) +
  theme_minimal() +
  ...
  labs(x = "P", y = "Coverage") +
  scale_color_discrete(name = "Variance Estimator", 
                       labels = c("Bootstrap", "Sandwich"))

# need to plot interactive effect
sand_preds2 <- data.frame(
  p_wrong = rep(seq(0, .15, length.out = 200), 3),
  p_error = rep(c(.025, .075, .125), each = 200),
  estimator = "sandwich",
  coverage = predict(
    sand, 
    data.frame(
...
boot_preds2 %>% 
  ...
  theme(panel.grid.minor.x = element_blank(),
        ...
        legend.position = "top",
        text = element_text(size = 14)) +
  labs(x = "Target Error (P)", y = "Coverage") +
  scale_color_discrete(name = "Measurement Error (P)")
