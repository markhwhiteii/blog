...
My hypothesis is that Aphex Twin has an especially eclectic discography. In statistical terms, we could characterize this as *the characteristics of the discography vary more than other artists.* So we want to look at each artist's discography and see how those variables mentioned above—danceability, energy, and valence—vary. We need a measure of multivariate spread or dispersion.  

When looking at just one variable, the answer is straightforward: We can look at the variance for each artist to characterize how much spread there is in the distribution of songs. This will be good enough metric for our uses, since all songs are measured on the same 0 to 1 scale.  

But we want to look at *three* variables. Summing up the variances of each of the three variables sounds appealing, but there's a problem: What if two of the variables are highly correlated? Imagine two situations where two variables are plotted against one another, one situation where the variables correlate highly (left panel) and one situation where the variables are completely uncorrelated (right panel).  
...
We can see that the points on the panel take up more space—they are more dispersed. We want a metric that appreciates this. Think of it this way: Since each of the three metrics from Spotify are on a 0 to 1 scale, think of a cube where each of the lengths are 1. In one corner of the box is a point representing a song that is high in energy, valence, and danceability. Another corner has songs where the danceability is high, valence is high, but energy is low. And so on.  
...
You can see all the data and code for this post by going to by GitHub.
